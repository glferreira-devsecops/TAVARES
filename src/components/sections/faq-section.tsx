...
export function FAQSection({ locale = "pt" }: FAQSectionProps) {
    const dict = useDictionary(locale);
    const [openIndex, setOpenIndex] = useState<string | null>("safety"); // Default open first

    const toggleAccordion = (key: string) => {
        setOpenIndex(openIndex === key ? null : key);
    };

    // Filter keys related to questions from the dictionary
    const questionKeys = Object.keys(dict.FAQ.questions) as Array<keyof typeof dict.FAQ.questions>;
                        ...
                        >
                            ...
                            <div className="p-8 bg-neutral-900/50 backdrop-blur-sm rounded-3xl border border-white/10 hover:border-white/20 transition-colors">
                                <div className="flex items-center gap-6 mb-6">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-white text-lg mb-1">{dict.NotFound.helpTitle}</h4>
                                        <p className="text-sm text-neutral-400">{dict.NotFound.helpSubtitle}</p>
                                    </div>
                                </div>
                                <a
                            ...
                            >
                                ...
                                >
                                    <span className={`text-lg md:text-xl font-bold transition-colors duration-300 ${openIndex === key ? "text-white" : "text-neutral-300 group-hover:text-white"
                                        ...
                                        {dict.FAQ.questions[key].question}
                                    </span>
