...
export function BookingModal({ isOpen, onClose, tourTitle, locale }: BookingModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            setStep(1);
            // Reset date but keep name if entered mostly for UX? No, reset all for fresh booking
            // Actually keeping name is nice if they close/reopen.
        }
    }, [isOpen]);
    ...
    const handleNext = () => {
        if (!data.date) return; // Add validation signal here later
        setStep(2);
    };
    ...
    const handleConfirm = () => {
        ...
        // Construct message
        // Template: "Olá! Quero reservar o tour "${tourName}" para ${date}, ${people} pessoa(s). Tem disponibilidade?"
        // Plus name intro.

        const baseMsg = WHATSAPP_MESSAGES.tourReservationWithDate(tourTitle, data.date, data.guests)[locale];
        const nameIntro = locale === "pt" ? `Olá! Me chamo ${data.name}. ` :
            locale === "es" ? `¡Hola! Me llamo ${data.name}. ` :
                locale === "fr" ? `Bonjour! Je m'appelle ${data.name}. ` :
                    `Hello! My name is ${data.name}. `;

        // Remove standard greeting from baseMsg if we add custom intro to avoid "Hello! Hello!"
        // Actually the template starts with greeting. Let's precise it.
        // Template: "Olá! Quero reservar..."
        // We can just append the reservation details to the intro.
            ...
            {isOpen && (
                <>
                    ...
                    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 pointer-events-none">
                        ...
                        >
                            {/* Header (Visual) */}
                            <div className="relative h-32 bg-neutral-900 flex items-center justify-center overflow-hidden shrink-0">
                                {/* Abstract Background */}
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full pl-12 pr-4 py-4 bg-neutral-50 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-medium text-neutral-900"
                                                    />
                                                </div>
                                            </div>
